...
class ConfigBase:
    ...
    FILELOG_FORMAT = u'[%(asctime)s] [PID: %(process)s] [{0}] %(levelname)s: %(message)s'.format(APPLICATION_NAME)
    SYSLOG_FORMAT = '%(isotime_utc)s %(hostname)s {0}[%(process)d]: %(levelname)s %(message)s'.format(APPLICATION_NAME)
    ADDITIONAL_LOGGERS = [
        'alembic',
        'werkzeug',
        # 'sqlalchemy',
    ...
    def read_config_file(self):
        ...
        self.FILELOG_LOGLEVEL = section.get('filelog_loglevel', fallback='debug')
        ...
        self.SYSLOG_SOCKET_TYPE = section.get('syslog_socket_type', fallback='UDP')
    ...
    def file_log_handler(self, file_path):
        ...
        handler.setLevel({'info': logging.INFO,
                          ...
                          'debug': logging.DEBUG,
                          'error': logging.ERROR,}[self.FILELOG_LOGLEVEL.lower()])
        return handler
    ...
    def syslog_log_handler(self):
        ...
        if self.SYSLOG_ADDRESS:
            ...
            handler = logging.handlers.SysLogHandler(
                ...
                socktype={'tcp': socket.SOCK_STREAM,
                          'udp': socket.SOCK_DGRAM,}[self.SYSLOG_SOCKET_TYPE.lower()]
            )
        ...
        handler.setLevel({'info': logging.INFO,
                          ...
                          'warning': logging.WARNING,
                          'debug': logging.DEBUG,
                          'error': logging.ERROR,}[self.SYSLOG_LOGLEVEL.lower()])
